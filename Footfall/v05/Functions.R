...
StoreModel <- function(x){
  x_train = x[1:(nrow(x)-120),]
  x_test = x[(nrow(x)-119):(nrow(x)),]
  ...
  return(list(model1, mse, x_train, x_test, err))
}
...
#   z <- count(y, c('Year','Week'))
#   j <- 1
#   for(i in 1:nrow(x)){
#     if(x$Year==z$Year & x$Week==z$Week){
#       x$NP <- z$freq
#       j <- j+1
#     }
#   }
